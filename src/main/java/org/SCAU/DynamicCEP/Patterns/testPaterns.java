...
//        System.out.println( new singlePattern(patternStr).toString());
    ...
    public static Pattern<stockSerializable,stockSerializable> Pattern3Control(){

        Pattern<stockSerializable,stockSerializable>  pattern= Pattern.<stockSerializable>begin("1").where(
                new SimpleCondition<stockSerializable>() {
                    @Override
                    public boolean filter(stockSerializable e) throws Exception {
                        return Float.parseFloat(e.getHigh())<5.5;
                ...
                new SimpleCondition<stockSerializable>() {
                    @Override
                    public boolean filter(stockSerializable e) throws Exception {
                        return Float.parseFloat(e.getLow())<5;
                    }
                }
        )
                ;
...
//                        System.out.println(e.toString());
    ...
    public static Pattern<stockSerializable,stockSerializable> Pattern6(){
        String patternStr = "\"1\":<org.SCAU.model.stockSerializable>[f:e.getHigh() < 19 & s:e.getSymbol() =FB]";
//        System.out.println( new singlePattern(patternStr).toString());
        Pattern<stockSerializable,stockSerializable> pattern = new singlePattern(patternStr).getPattern();
        return pattern;
